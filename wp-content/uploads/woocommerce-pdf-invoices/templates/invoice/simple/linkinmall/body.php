...
$postmetadate                   = $order->get_meta( 'billing_datetime' );
$postmetatime                   = $order->get_meta( 'billing_timeget' );
$postmetaadult                  = $order->get_meta( 'billing_adult' );
$postmetachildren               = $order->get_meta( 'billing_children' );
$postmetababy                   = $order->get_meta( 'billing_baby' );
?>
...
<table cellpadding="0" cellspacing="0">
	...
	foreach ( $invoice->get_columns_data() as $index => $row ) {
		...
		?>
		<td>日期: <?php echo $postmetadate; ?><br />
		時間: <?php echo $postmetatime; ?><br />
		<?php if ( $postmetaadult >0 ){ echo '成人: '.$postmetaadult; } ?><br />
		<?php if ( $postmetachildren >0 ){ echo '兒童: '.$postmetachildren; } ?><br />
		<?php if ( $postmetababy >0 ){ echo '幼兒: '.$postmetababy; } ?>
		<?php
